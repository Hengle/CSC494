...
namespace Utilities
{
    ...
    public static class TransformUtilities
    {
        ...
        public static void SetGlobalScale(this Transform transform, float3 globalScale)
        {
            transform.localScale = Vector3.one;
            transform.localScale = globalScale / math.max(math.abs(transform.lossyScale), 1e-7f);
        }
    ...
    public class AnimationHelper : MonoBehaviour
    {
        ...
        void UpdatePositions()
        {
            ...
            foreach ((Transform tf, float3 target) in positions)
            {
                ...
                if (math.lengthsq((float3)tf.position - target) < 1e-5)
                {
                    tf.position = target;
                    completed.Add(tf);
                }
        ...
        void UpdateRotations()
        {
            ...
            foreach ((Transform tf, Quaternion target) in rotations)
            {
                ...
                if (Quaternion.Angle(tf.rotation, target) < 0.1f)
                {
                    tf.rotation = target;
                    completed.Add(tf);
                }
        ...
        void UpdateLocalRotations()
        {
            ...
            foreach ((Transform tf, Quaternion target) in localRotations)
            {
                ...
                if (Quaternion.Angle(tf.localRotation, target) < 0.1f)
                {
